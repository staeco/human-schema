...
const isValidDate = (v) => {
  ...
  const parsed = moment_timezone_1.default(
    v,
    moment_timezone_1.default.ISO_8601
  )
  return parsed.isValid() && parsed.toISOString() === v
}
const isNumber = (v) => typeof v === 'number' && Number.isFinite(v)
const getBasicGeoJSONIssues = (v, type) => {
  if (!is_plain_obj_1.default(v)) return 'Not a valid object'
  if (v.type !== type)
...
exports.text = {
  ...
  validators: {
    ...
    stream: {
      name: 'Stream URL',
      validateValue: (param) => param === true,
      test: (v) => utils_1.isValidURL(v, ['http', 'https', 'rtmp', 'rtmps']),
      valueType: 'boolean'
    },
    email: {
      name: 'Email',
...
exports.point = {
  ...
  testAsync: async (v, conn) => {
    ...
    if (conn) return isValidGeometry_1.default(v, conn)
    return true
  },
  validators: {
    required,
